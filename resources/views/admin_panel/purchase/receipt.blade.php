...
<body>
    ...
    <div class="header">
        <div class="company-name">Three Star Medical</div>
            ...
            @foreach ($purchase->items as $item)
                ...
                <tr>
                    <td colspan="3" style="font-weight: 600;">
                        {{ $item->product->item_name ?? 'Item' }} {{ $item->product->brand->name ?? '' }}
                        @if(!empty($purchase->enable_hs_code) && (!empty($item->hs_code) || !empty($item->product->hs_code)))
                            <br><span style="font-size:9px; font-weight:normal; color:#555;">HS: {{ $item->hs_code ?: $item->product->hs_code }}</span>
                        @endif
                    </td>
                </tr>
                <tr>

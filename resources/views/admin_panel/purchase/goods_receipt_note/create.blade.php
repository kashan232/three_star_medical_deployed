            ...
            <div class="container-fluid py-4">
                ...
                <form id="purchaseForm" action="{{ route('store.Purchase') }}" method="POST" autocomplete="off">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h1 class="page-title text-success"><i class="bi bi-patch-check me-2"></i>
                            Goods Receipt Note (GRN)
                        </h1>
                        <p class="page-subtitle mb-0">Post final stock arrivals and update inventory status.</p>
                        <div class="d-flex gap-3">
                            ...
                            <a href="{{ route('purchase.grn.index') }}" class="btn-erp btn-erp-secondary">
                                <i class="bi bi-arrow-left"></i> Back to GRN List
                            </a>
                            <!-- Shortcut Actions -->
                        </div>
                    </div>
                    ...
                    <div class="erp-card mb-3">
                        <div class="erp-card-header">
                            <h3 class="erp-card-title text-success"><i class="bi bi-check2-circle"></i>
                                GRN Verification Information
                            </h3>
                                                ...
                                                <tr style="display: none;">
                                                    ...
                                                    <td colspan="3" class="compact-td">
                                                        <input type="text" class="form-control fw-bold shadow-sm"
                                                            ...
                                                            disabled>
                                                        <input type="hidden" name="warehouse_id" id="main_warehouse_id"
                                                            value="{{ $Warehouse->first()->id ?? '' }}">
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="compact-lbl compact-td">GRN #</td>
                                                    <td class="text-success fw-bold compact-td" style="width: 35%;">
                                                        <input type="hidden" name="invoice_no"
                                                            value="{{ $nextInvoice ?? 'GRN-0001' }}">
                                                        <input type="text" readonly
                                                            value="{{ $nextInvoice ?? 'GRN-0001' }}"
                                                            class="bg-transparent border-0 text-success p-0 w-100 fw-bold"
                                                            style="outline: none;">
                                                    </td>
                                ...
                                <table class="erp-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width:160px; display:none;">Warehouse</th>
                                            ...
                                            <th style="min-width:150px" class="text-end">Net Total</th>
...
<script>
    $(document).ready(function() {
        ...
        function recalcRow($row) {
            ...
            const gstAmount  = subTotal * (gstPercent / 100);  // GST ADDED
        ...
        function recalcSummary() {
            ...
            $('#purchaseItems tr').each(function() {
                ...
                totalGstAmt += rowGst; // Only GST contributes to gstAmt total
            });
            ...
            const newRow = `
              <tr class="item-row">
                ...
                <td style="display:none;">
                    <select name="item_warehouse_id[]" class="form-select item-warehouse row-input" style="min-width:120px;">
                        ${warehouseOptions}
                    </select>
                </td>
        ...
        function initSelect2($el) {  /* kept for legacy */
            ...
            $el.on('select2:select', function(e) {
                populateProductRow($(this).closest('tr'), e.params.data, false);
            });

            // Header Warehouse Sync: When header warehouse changes, update all existing rows
            ...
            $el.closest('tr').on('change', '.item-uom-select', async function() {
                ...
                if (val === 'new_uom') {
                    ...
                    if (formValues) {
                        ...
                        // Create a temporary hidden option for this "new" UOM
                        var $opt = $('<option>', {
                            value: 'NEW',
                            ...
                            'data-is-new': 'true',
                            'data-name': newName,
                            'data-price': 0 // new uom, use existing or user will type
                        });
                    ...
                    // If UOM has a price defined, update the row price
        ...
        function populateProductRow($row, data, isImport = false) {
            ...
            var btnText = (data.item_name || 'Select Product') + (data.item_code ? ' (' + data.item_code + ')' : '');
            ...
            // Set product default price and factor immediately
            $row.find('.price').val(data.price);
            $row.find('.item-uom').val(data.uom || 1);
            ...
            items.forEach(function(item) {
                ...
                $.getJSON('/productview/' + item.product_id, function(product) {
                    ...
                    const mappedData = {
                        ...
                        price: item.price || product.purchase_price_per_piece || 0,
                    ...
                    // populateProductRow will automatically fetch last price if needed
                    populateProductRow($lastRow, mappedData, true);

                    // Force set the specific packing from PO if possible
                    if (item.uom_id) {
                        $lastRow.find('.item-uom-select').val(item.uom_id).trigger('change');
                    }

                    // Ensure warehouse is set
                    let wh = $('#main_warehouse_id').val();
                    if (wh) $lastRow.find('.item-warehouse').val(wh);

                    // Sync stock for the selected warehouse
                    if (wh) {
                        $.get("{{ url('product-warehouse-stock') }}/" + product.id + "/" + wh, function(stock) {
                            $lastRow.find('.stock-label').text((stock || 0) + ' Pcs');
                        });
                    }
                    ...
                    setTimeout(() => {
                        ...
                        let l = parseFloat(item.loose_qty) || 0;
                        let f = parseFloat(item.free_qty_pieces || 0);
                        
                        $lastRow.find('.quantity').val(q);
                        $lastRow.find('.loose_qty').val(l);
                        $lastRow.find('.free_qty_pieces').val(f);
        ...
        function initPackingSelect2($el, $row) {
            $el.select2({
                ...
                if ($opt.data('factor')) {
                    ...
                    // New Packing (On the fly)
                    $row.find('.item_uom_factor').val(1); // Default factor for new

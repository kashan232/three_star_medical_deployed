@php
    $typeLabel = "CASH RECEIPT VOUCHER";
    $vNo = $voucher->rvid ?? $voucher->voucher_no ?? '-';
    ...
    foreach($rows as $r) {
        ...
        if (!isset($r['debit']) && !isset($r['credit'])) {
             // In Receipt Voucher, rows are usually Credit to Party/Account and Debit to Cash/Bank
             // But the 'rows' passed to this view are usually the LINE ITEMS.
             // Receipt lines are typically CREDIT to the income/customer account.
             $totalCredit += $r['amount'];
        }
    }
    
    // Add the counter-party row if not present
    // For Receipt, Total Amount is DEBITED to Cash/Bank
    $needsContraDebit = ($totalDebit == 0 && $voucher->total_amount > 0);
    if ($needsContraDebit) {
        $totalDebit = $voucher->total_amount;
    }
    if ($totalCredit == 0 && $voucher->total_amount > 0) {
    ...
    <tbody>
      ...
      <tr class="item-sub">
        ...
        <td class="r">
            ...
            @elseif(isset($row['type']) && $row['type'] == 'credit' || !isset($row['debit']))
                {{ number_format($row['amount'], 2) }}
            @else
                0.00
            @endif

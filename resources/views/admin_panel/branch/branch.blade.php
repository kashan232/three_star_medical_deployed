 ...
 @section('content')
                                         ...
                                         <tbody class="text-center">
                                             @foreach ($branches as $branch)
                                                 <tr>
                                                     ...
                                                     <td class="branch-code">{{ $branch->branch_code ?? 'N/A' }}</td>
                                                     ...
                                                     <td class="users-count">
                                                         <span class="badge bg-info text-dark">{{ $branch->users_count }}
                                                             Users</span>
                                                     </td>
                                                     <td>
                                                         @can('branches.edit')
                     ...
                     <form class="myform" action="{{ route('branch.store') }}" method="POST">
                         ...
                         <div class="mb-3 form-check">
                             <input type="checkbox" name="is_active" class="form-check-input" id="is_active" value="1"
                                 checked>
                             <label class="form-check-label" for="is_active">Branch is Active</label>
                         </div>
                 </div>
     ...
     <script>
         ...
         $(document).on('click', '.edit-btn', function() {
             ...
             var branchCode = tr.find(".branch-code").text();
             var name = tr.find(".name").text();
             var address = tr.find(".address").text();
             var number = tr.find(".number").text();
             var statusText = tr.find(".status").text().trim();

             $('#id').val(id);
             $('#branch_code').val(branchCode !== 'N/A' ? branchCode : '');
             $('#name').val(name)
             $('#address').val(address)
             $('#number').val(number)
             $('#is_active').prop('checked', statusText === 'Active');

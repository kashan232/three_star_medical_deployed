            ...
            <form action="{{ route('vendor.payments.store') }}" method="POST">@csrf
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-2">
                            <label>Purchase Reference (Optional)</label>
                            <select name="purchase_id" id="purchase_id" class="form-control">
                                <option value="">-- No Purchase (General Ledger Payment) --</option>
                                @foreach ($purchases as $purchase)
                                    <option data-vendor="{{ $purchase->vendor_id }}" value="{{ $purchase->id }}"
                                        style="display:none;">
                                        {{ $purchase->invoice_no }} (Due: {{ number_format($purchase->due_amount, 2) }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
    ...
    $(document).ready(function() {
        $('#vendor_id').on('change', function() {
            ...
            $('#purchase_id option').each(function() {
                if ($(this).val() === "") {
                    $(this).show();
                } else if ($(this).attr('data-vendor') == vendorId) {
                    $(this).show();
                } else {
                    $(this).hide();
                }

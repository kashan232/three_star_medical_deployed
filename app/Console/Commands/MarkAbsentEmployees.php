...
class MarkAbsentEmployees extends Command
{
    ...
    public function handle()
    {
        ...
        // We will check holidays per employee now
        ...
        foreach ($employees as $employee) {
            $isWeeklyOff = $employee->weekly_off && strtolower($employee->weekly_off) === $dayName;
            
            // Check if it's a holiday or weekly off for this employee
            if (Holiday::isHoliday($date, $employee->id) || $isWeeklyOff) {
                // If the employee is off due to a holiday or weekly off, we don't count it as a casual/sick leave day
                // and we don't mark them as absent.
                $reason = $isWeeklyOff ? 'Weekly Off' : 'Holiday';
                $this->line("Skipping: {$employee->full_name} is off due to ({$reason})");
                continue;
            }

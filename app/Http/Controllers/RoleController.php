...
class RoleController extends Controller
{
     public function index()
    {
        ...
        if (auth()->user()->isSuperAdmin()) {
            ...
            $allPermissions = Permission::where('name', 'not like', 'branches.%')->get();
        }
    ...
    public function updatePermissions(Request $request)
    {
        ...
        if (!auth()->user()->isSuperAdmin()) {
            $existingBranchPerms = $role->permissions()->where('name', 'like', 'branches.%')->pluck('name')->toArray();
            $requestedPermissions = array_filter($requestedPermissions, fn($p) => !\Illuminate\Support\Str::startsWith($p, 'branches.'));
            $requestedPermissions = array_merge($requestedPermissions, $existingBranchPerms);
        }

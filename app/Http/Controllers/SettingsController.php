...
class SettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin_panel.settings.index', compact('settings', 'users'));
    }
    ...
    public function update(Request $request)
    {
        ...
        // We need to identify any multiple-choice settings that might be empty
        // If a key is expected to be an array but is missing, we should clear it.
        $allKeys = Setting::pluck('key')->toArray();
        
        foreach ($allKeys as $key) {
            $setting = Setting::where('key', $key)->first();
            if ($setting && $setting->type === 'json') {
                // If it's a JSON/Array type and NOT in the request, set to empty array
                $value = $settingsData[$key] ?? [];
                Setting::set($key, $value);
            } elseif (isset($settingsData[$key])) {
                Setting::set($key, $settingsData[$key]);
            }
        }
